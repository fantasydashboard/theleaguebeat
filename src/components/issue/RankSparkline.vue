<template>
  ...
  >
    ...
    <!-- Endpoint dots + team labels for focus teams. The dot anchors
         the current state; the label identifies which line belongs
         to which team without needing a separate legend. -->
    <g v-for="line in focusLines" :key="`end-${line.id}`">
      <circle
        :cx="line.lastX"
        :cy="line.lastY"
        r="7"
      ...
      <circle
        ...
        opacity="0.4"
      />
      <!-- Endpoint team label — sits just to the right of the dot,
           or above when the dot is near the right edge of the plot. -->
      <text
        :x="line.labelX"
        :y="line.labelY"
        :fill="line.color"
      ...
      >{{ line.teamName }} <tspan class="rank-spark-team-rank">#{{ line.lastRank }}</tspan></text>
    </g>
...
function buildLine(teamId: string, color: string, teamName: string): SparkLine | null {
  ...
  // Label sits to the right of the dot by default, flipped to the
  // left when the dot is too close to the right edge to fit text.
...
const focusLines = computed<SparkLine[]>(() => {
  ...
  return props.focusTeamIds
    .map((id, i) => {
      ...
      return buildLine(id, colors[i % colors.length], name)
    })
