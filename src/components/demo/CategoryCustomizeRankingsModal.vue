      ...
      >
        <header class="cr-head">
          <div>
            ...
            <p id="cr-sub" class="cr-sub">Adjust how each category factor weighs your standings.</p>
          </div>
          <button
            ref="closeBtnRef"
            type="button"
          ...
          <ul class="cr-factors" role="list">
            ...
            >
              ...
              <div class="cr-factor-id">
                <span class="cr-factor-dot" aria-hidden="true"></span>
                <div class="cr-factor-text">
                  <p class="cr-factor-name">{{ f.label }}</p>
                  <p class="cr-factor-desc">{{ f.description }}</p>
                </div>
              </div>

              <div class="cr-factor-control">
                <label class="cr-slider-wrap" :aria-label="`Weight for ${f.label}`">
                  <input
                    type="range"
                    class="cr-slider"
                    min="0"
                ...
                <button
                  ...
                  :aria-label="`${enabled[f.id] ? 'Disable' : 'Enable'} ${f.label}`"
...
import { factorDefs, categoryPresets, type CategoryFactorScores } from '@/fixtures/categoriesLeague'
...
function onSlider(e: Event, id: keyof CategoryFactorScores) {
  const target = e.target as HTMLInputElement
  setWeight(id, Number(target.value))
}

// The currently-leading (highest-weight) enabled factor gets the visual accent.
const leaderFactorId = computed<keyof CategoryFactorScores | null>(() => {
  let bestId: keyof CategoryFactorScores | null = null
  let bestVal = -1
...
function sliderStyle(id: keyof CategoryFactorScores) {
...
.cr-preset:active { transform: scale(0.97); transition-duration: 100ms; }
.cr-preset:focus-visible { outline: 2px solid var(--accent-primary); outline-offset: 2px; }
.cr-preset-active {
  color: oklch(0.96 0.10 195);
  background: oklch(0.72 0.18 195 / 0.14);
  border-color: oklch(0.72 0.18 195 / 0.55);
}
...
.cr-factor + .cr-factor { border-top: 1px solid oklch(0.16 0.015 90); }
.cr-factor-stripe {
  position: absolute;
  left: 0;
  top: 6px;
  ...
  width: 2px;
  border-radius: 2px;
  background: var(--factor-color, var(--accent-tertiary));
  opacity: 0;
...
.cr-factor-leader { background: oklch(0.72 0.18 195 / 0.04); }
...
/* Slider */
.cr-slider-wrap { display: block; width: 100%; }
.cr-slider {
  appearance: none;
...
.cr-switch:focus-visible { outline: 2px solid var(--accent-primary); outline-offset: 2px; border-radius: 999px; }
.cr-switch:active { transform: scale(0.97); transition-duration: 100ms; }
.cr-switch-track {
  position: relative;
  display: block;
  width: 30px;
...
.cr-switch-on { background: var(--accent-tertiary); }
...
.cr-reset:active { transform: scale(0.97); transition-duration: 100ms; }
...
.cr-done:active { transform: scale(0.97); transition-duration: 100ms; }
